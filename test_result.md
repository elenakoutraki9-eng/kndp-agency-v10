...
test_plan:
  current_focus:
    - "Mobile scroll performance at Hero -> Problems transition"
  stuck_tasks: []
  test_all: false
...
agent_communication:
    - agent: "main"
      message: "BUG FIX to verify (MOBILE 390x844 primary; also sanity-check desktop 1440x900). User reported the scroll transition from the Hero section to the section immediately below it (Problems) feels laggy on mobile. I removed/gated the heavy work at that scroll point: (a) disabled the Hero's scroll-linked parallax on the big ChatDesktop mockup on mobile (no y-transform, no will-change), (b) removed the infinite float loop on the hero floating card on mobile and disabled backdrop-blur-xl/2xl/3xl via CSS on mobile, (c) idled the Problems arrow infinite bounce on mobile. Please: (1) On mobile emulation, load the site (an intro overlay plays first, ~1.25s, then reveals the site — wait for data-testid='intro-overlay' to disappear), then scroll slowly from the Hero (data-testid='hero-section') down into the Problems section (data-testid='problem-solution-section') and confirm the section renders correctly, the 4 rows (problem-solution-row-0..3) appear, NO console errors, and NO horizontal overflow. (2) Verify the hero mockup does not apply a scroll parallax y-offset on mobile (should stay put relative to its column as you scroll). (3) Confirm desktop still works and the hero parallax + animations remain on desktop. Note the site uses native scroll on mobile (Lenis disabled on mobile)."
    - agent: "main"
